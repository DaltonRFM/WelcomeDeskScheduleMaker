...
- [x] **Step 2** — Write a parser that turns an exported availability sheet
      (CSV) into the data model — no Google API auth yet, just local files
- [x] **Step 3** — Hard-code a tiny fake dataset (3–4 people, 1 day) and get
      OR-Tools CP-SAT producing *any* valid schedule respecting availability
      + operating hours
- [x] **Step 4** — Add station coverage requirements (North/South/Dean's
      staffed correctly)
- [x] **Step 5** — Add open/close-at-least-once and min/max weekly hours
      constraints
- [x] **Step 6** — Add the "must work North and South at some point" rule
- [x] **Step 7** — Add soft constraints: honor requests, minimize
